...
exports.createReport = async (req, res) => {
  try {
    const { patientId, doctorId, results, reportDate } = req.body;
...
exports.updateReport = async (req, res) => {
  try {
    ...
    // If status is updated to COMPLETED, check if patient has linked Telegram chat and dispatch report
    if (status === 'COMPLETED') {
      ...
      if (fullReport && fullReport.patient && fullReport.patient.telegramChatId) {
        // Run in background asynchronously so it doesn't block the API response
        (async () => {
          try {
            const { generateReportPDF } = require('../utils/pdfGenerator');
            const { sendReportToTelegram, generateReportSummary } = require('../utils/telegramBot');
            
            const pdfBuffer = await generateReportPDF(fullReport);
            const summaryText = await generateReportSummary(fullReport);
            
            await sendReportToTelegram(fullReport.patient.telegramChatId, pdfBuffer, fullReport.reportNumber, summaryText);
          } catch (tgErr) {
            console.error('Failed to auto-dispatch PDF report to Telegram:', tgErr.message);
          }
        })();
      }
    }
