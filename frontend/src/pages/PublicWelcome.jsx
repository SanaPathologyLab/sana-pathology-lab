  ...
  return (
    <div className="min-h-screen bg-bg relative font-sans text-slate-800 scroll-smooth">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-8 mr-4">
              ...
              <a href="#packages" onClick={(e) => { e.preventDefault(); scrollToSection('packages'); }} className="text-sm font-bold text-slate-600 hover:text-primary transition-colors">{t('packages')}</a>
              <a href="#services" onClick={(e) => { e.preventDefault(); scrollToSection('services'); }} className="text-sm font-bold text-slate-600 hover:text-primary transition-colors">{t('testFinder')}</a>
      ...
      <section className="relative pt-24 pb-32 lg:pt-32 lg:pb-40 overflow-hidden bg-[#085041]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-8 lg:gap-12 text-white">
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/10 backdrop-blur-sm"><CheckCircle2 className="text-[#F1C40F]" size={20} /><span className="font-medium">{t('pts15k')}</span></div>
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/10 backdrop-blur-sm"><Clock className="text-[#F1C40F]" size={20} /><span className="font-medium">{t('tat612')}</span></div>
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/10 backdrop-blur-sm"><Phone className="text-[#F1C40F]" size={20} /><span className="font-medium">{t('freeHomeCollection')}</span></div>
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/10 backdrop-blur-sm"><ShieldCheck className="text-[#F1C40F]" size={20} /><span className="font-medium">{t('nablAccredited')}</span></div>
          </div>
        </div>
      </section>
      ...
      <section ref={statsRef} id="stats" className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500">{t('statsSub')}</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { label: t('patientsServed'), value: counterValues.patients.toLocaleString() + '+', icon: <UserCircle className="w-6 h-6" />, color: 'text-[#085041]', bg: 'bg-[#085041]/5' },
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            {[
              { icon: <ShieldCheck className="w-4 h-4" />, label: t('nablAccredited'), color: 'text-green-700 bg-green-50 border-green-200' },
              { icon: <Award className="w-4 h-4" />, label: t('iso15189'), color: 'text-blue-700 bg-blue-50 border-blue-200' },
              { icon: <Clock className="w-4 h-4" />, label: t('tat612hr'), color: 'text-amber-700 bg-amber-50 border-amber-200' },
              { icon: <Heart className="w-4 h-4" />, label: t('compassionateCare'), color: 'text-red-600 bg-red-50 border-red-200' },
              { icon: <Zap className="w-4 h-4" />, label: t('digitalReports'), color: 'text-purple-700 bg-purple-50 border-purple-200' },
              { icon: <Phone className="w-4 h-4" />, label: t('freeHomeCollection'), color: 'text-teal-700 bg-teal-50 border-teal-200' },
            ].map((badge, i) => (
      ...
      <section id="search-section" className="py-8 px-4 sm:px-6 lg:px-8 bg-slate-50">
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-2xl font-bold text-slate-800">{t('checkReportsBookings')}</h3>
              <p className="text-slate-500 text-sm mt-1">{t('searchDesc')}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            ...
            >
              ...
              {searchType === 'appointment' ? t('trackRequest') : t('findReport')}
            </button>
          </form>
              ...
              {appointmentResults.length > 0 && (
                <div className="space-y-4">
                  <h4 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider">
                    {t('foundRequests').replace('{count}', appointmentResults.length)}
                  </h4>
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                    {appointmentResults.map((apt) => {
                      ...
                      const statusMap = {
                        SCHEDULED: { text: `⏳ ${t('pendingConfirmation')}`, cls: 'bg-amber-50 text-amber-800 border-amber-200' },
                        CONFIRMED: { text: `✓ ${t('confirmedScheduled')}`, cls: 'bg-blue-50 text-blue-800 border-blue-200' },
                        COMPLETED: { text: `✓ ${t('sampleCollected')}`, cls: 'bg-emerald-50 text-emerald-800 border-emerald-200' },
                        CANCELLED: { text: `❌ ${t('cancelled')}`, cls: 'bg-red-50 text-red-800 border-red-200' }
                      };
                        ...
                        <div key={apt.id} className="bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-primary-light transition-colors animate-fade-in-up">
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-400">
                              ...
                              <span>•</span>
                              <span className="flex items-center gap-1">
                                <MapPin size={12} /> {isHome ? t('freeHomeCollection') : t('visitLab')}
                              </span>
                            </div>
                          </div>
                          ...
                          {apt.status === 'SCHEDULED' && (
                            ...
                            >
                              <MessageCircle size={14} /> {t('contactLab')}
                            </button>
                          )}
                        </div>
                      );
      ...
      <section id="stats" className="py-20 bg-bg px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-heading text-primary">{t('atAGlance')}</h2>
            <p className="text-slate-500 mt-2 max-w-xl mx-auto">{t('glanceSub')}</p>
            <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
          </div>
          
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {[
              { title: '15,000+', desc: t('glancePts'), color: 'border-emerald-500', icon: <UserCircle className="w-8 h-8 text-emerald-600" /> },
              { title: '250+', desc: t('glanceTests'), color: 'border-blue-500', icon: <Microscope className="w-8 h-8 text-blue-600" /> },
              { title: '6 Hours', desc: t('glanceDelivery'), color: 'border-amber-500', icon: <Clock className="w-8 h-8 text-amber-600" /> },
              { title: '100%', desc: t('glanceAccurate'), color: 'border-red-500', icon: <Award className="w-8 h-8 text-red-600" /> },
            ].map((item, i) => (
      ...
      <section className="py-20 bg-primary-pale/30 px-4 sm:px-6 lg:px-8 border-y border-emerald-100">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-xs font-bold tracking-widest text-primary uppercase bg-primary-pale px-3 py-1 rounded-full">{t('process')}</span>
            <h2 className="text-4xl font-heading text-primary mt-2">{t('howItWorks')}</h2>
            <p className="text-slate-500 mt-2">{t('processSub')}</p>
            <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
          </div>
      ...
      <section id="packages" className="py-20 bg-bg px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-xs font-bold tracking-widest text-[#BA7517] uppercase bg-accent-pale px-3 py-1 rounded-full">{t('sanaCare')}</span>
            <h2 className="text-4xl font-heading text-primary mt-2">{t('popularPackages')}</h2>
            <p className="text-slate-500 mt-2 max-w-2xl mx-auto">{t('packagesSub')}</p>
            <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
          </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-xs font-extrabold text-[#BA7517] bg-accent-pale px-2.5 py-1 rounded-full uppercase">
                        {t('badge' + pkg.badge.replace(/\s+/g, '')) === 'badge' + pkg.badge.replace(/\s+/g, '') ? pkg.badge : t('badge' + pkg.badge.replace(/\s+/g, ''))}
                      </span>
                      <span className="text-xs text-slate-400 font-bold uppercase">{t('catalog')}</span>
                    </div>
                    <h3 className="text-xl font-bold text-slate-800 mb-2 group-hover:text-primary transition-colors">{t(pkg.code + '_name') === pkg.code + '_name' ? pkg.name : t(pkg.code + '_name')}</h3>
                    <p className="text-slate-500 text-xs mb-4 leading-relaxed">{t(pkg.code + '_desc') === pkg.code + '_desc' ? pkg.desc : t(pkg.code + '_desc')}</p>

                    {/* Includes list */}
                    <div className="border-t border-slate-100 pt-4 mb-6">
                      <p className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-1">
                        <BadgePercent size={14} className="text-primary" /> {t('includesTests').replace('{count}', pkg.tests.length)}
                      </p>
                      <ul className="space-y-1.5">
                        {pkg.tests.map((testName, idx) => (
                          <li key={idx} className="text-slate-600 text-xs flex items-center gap-1.5 font-medium">
                            ...
                            <span className="truncate">{t(testName) || testName}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
      ...
      <section id="services" className="py-20 bg-primary-pale/25 px-4 sm:px-6 lg:px-8 border-y border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <span className="text-xs font-bold tracking-widest text-primary uppercase bg-primary-pale px-3 py-1 rounded-full">{t('catalog')}</span>
            <h2 className="text-4xl font-heading text-primary mt-2">{t('testExplorer')}</h2>
            <p className="text-slate-500 mt-2 max-w-xl mx-auto">{t('explorerSub')}</p>
            <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
          </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-slate-100 space-y-4">
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                  ...
                  {categories.map((cat, i) => (
                    ...
                    >
                      {t('filter' + cat) === 'filter' + cat ? cat : t('filter' + cat)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {loadingTests ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-slate-100">
                  ...
                  <p className="text-slate-500 text-sm font-medium mt-4">{t('submittingBooking')}</p>
                </div>
                ...
                <div className="p-12 text-center bg-white rounded-2xl border border-slate-100">
                  ...
                  <h3 className="text-lg font-bold text-slate-700">{t('noTestsSelected')}</h3>
                  <p className="text-slate-500 text-sm mt-1">{t('noTestsMatchingDesc').replace('{query}', searchQueryTest)}</p>
                  <button 
                        ...
                        <div className="flex-1 min-w-0 pr-4">
                          <div className="flex items-center gap-2 mb-1.5">
                            <span className="text-[10px] font-bold text-primary bg-primary-pale px-2 py-0.5 rounded uppercase tracking-wider">
                              {t('filter' + (test.category?.name || 'Other')) === 'filter' + (test.category?.name || 'Other') ? (test.category?.name || 'Test') : t('filter' + (test.category?.name || 'Other'))}
                            </span>
                            <span className="text-[10px] text-slate-400 font-medium tracking-wide">
                              {t('code')}: {test.testCode}
                            </span>
                          </div>
                          <h4 className="font-bold text-slate-800 text-sm sm:text-base group-hover:text-primary transition-colors truncate">
                            {t(test.testCode) === test.testCode ? test.testName : t(test.testCode)}
                          </h4>
                          <div className="flex items-center gap-3 mt-2 text-xs text-slate-500">
                            ...
                            <span>{(t(test.sampleType) || test.sampleType || 'Blood')} {t('sample')}</span>
                          </div>
                        </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-md border border-slate-100 sticky top-24 space-y-6">
              ...
              {selectedTests.length === 0 ? (
                <div className="py-12 text-center text-slate-400">
                  ...
                  <p className="text-xs font-semibold">{t('noTestsSelected')}</p>
                  <p className="text-[11px] mt-1">{t('clickToSelect')}</p>
                </div>
              ) : (
                <>
                  <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                    {selectedTests.map((tItem, idx) => (
                      <div key={idx} className="flex justify-between items-start gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100 relative group">
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-slate-800 truncate">
                            {tItem.isPackage ? (t(tItem.testCode + '_name') === tItem.testCode + '_name' ? tItem.name : t(tItem.testCode + '_name')) : (t(tItem.testCode) === tItem.testCode ? tItem.name : t(tItem.testCode))}
                          </p>
                          <p className="text-[10px] text-slate-500 font-medium mt-0.5">₹{tItem.price} • {tItem.isPackage ? t('packages') : t('catalog')}</p>
                        </div>
                        <button 
                          onClick={() => removeSelectedTest(tItem.testCode)}
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-500 font-bold">{t('totalTests')}:</span>
                      <span className="font-extrabold text-slate-800">{selectedTests.length}</span>
                    </div>
                    <div className="flex justify-between items-center text-base">
                      <span className="text-primary font-bold">{t('estimatedCost')}:</span>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div className="bg-primary p-6 md:p-8 text-white relative">
              ...
              <h2 className="text-2xl md:text-3xl font-heading mt-3">{t('bookHomeSample')}</h2>
            ...
            <div className="p-6 md:p-8">
              ...
              {bookingSuccess ? (
                <div className="text-center py-8 animate-fade-in-up">
                  ...
                  <h3 className="text-2xl font-bold text-slate-800 mb-2">{t('requestSubmitted')}</h3>
                  <p className="text-slate-600 mb-6 max-w-md mx-auto text-sm">
                    {t('thankYouDesc').replace('{name}', bookingForm.name)}
                  </p>
                  
                  {createdAppointment && (
                    <div className="mb-8 bg-primary-pale/50 border border-primary/15 rounded-2xl p-4 max-w-md mx-auto">
                      <p className="text-[10px] font-extrabold text-primary uppercase tracking-wider">{t('trackingRefId')}</p>
                      <p className="text-2xl font-black text-primary mt-1">SPL-APT-{createdAppointment.id.toString().padStart(6, '0')}</p>
                      <p className="text-[11px] text-slate-500 mt-2 font-medium">{t('trackInstruction')}</p>
                    </div>
                  )}
                ...
                <form onSubmit={handleHomepageBookSubmit} className="space-y-6">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                    ...
                    {selectedTests.length === 0 ? (
                      <div className="text-sm text-slate-500 py-1.5 flex items-center gap-1.5">
                        ...
                        <span>{t('noSelectedTestsWarning')}</span>
                      </div>
                    ) : (
                      <div className="flex flex-wrap gap-1.5 max-h-[120px] overflow-y-auto pr-1">
                        {selectedTests.map((tItem, idx) => (
                          <span key={idx} className="inline-flex items-center gap-1 bg-primary-pale text-primary text-xs font-bold px-3 py-1.5 rounded-lg border border-primary/10">
                            {tItem.isPackage ? (t(tItem.testCode + '_name') === tItem.testCode + '_name' ? tItem.name : t(tItem.testCode + '_name')) : (t(tItem.testCode) === tItem.testCode ? tItem.name : t(tItem.testCode))} (₹{tItem.price})
                            <button 
                              type="button" 
                              onClick={() => removeSelectedTest(tItem.testCode)}
                    ...
                    {selectedTests.length > 0 && (
                      <div className="flex justify-between items-center pt-2.5 border-t border-slate-200 text-sm">
                        <span className="text-slate-500 font-semibold">{t('totalPrice')}:</span>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                        <MapPin size={16} />
                        <span>{t('homeCollectionFree')}</span>
                      </button>
                      <button
                        type="button"
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">{t('certifiedSignatures')}</h4>
                  <p className="text-slate-500 text-xs mt-0.5">{t('signaturesDesc')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="text-primary mt-1 shrink-0" size={18} />
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">{t('nablAudits') || 'NABL Quality Audits'}</h4>
                  <p className="text-slate-500 text-xs mt-0.5">{t('auditsDesc')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="text-primary mt-1 shrink-0" size={18} />
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">{t('barcodedSpecimen')}</h4>
                  <p className="text-slate-500 text-xs mt-0.5">{t('specimenDesc')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="text-primary mt-1 shrink-0" size={18} />
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">{t('automatedAnalyzers')}</h4>
                  <p className="text-slate-500 text-xs mt-0.5">{t('analyzersDesc')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-3xl p-8 shadow-sm border border-emerald-50 max-w-md w-full relative">
            ...
            <div className="flex items-center gap-4 pb-6 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs font-semibold">{t('chiefPathologist')}</p>
                <p className="text-slate-400 text-[10px] mt-0.5">{t('regNo')}</p>
              </div>
            </div>

            <div className="py-6 space-y-4">
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold">{t('verification')}</span>
                <span className="text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded">{t('activeVerified')}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold">{t('specialization')}</span>
                <span className="text-slate-700 font-bold">{t('specDetails')}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold">{t('experience')}</span>
                <span className="text-slate-700 font-bold">{t('expDetails')}</span>
              </div>
            </div>
      ...
      <section className="py-20 bg-bg px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-heading text-primary">{t('patientTestimonials')}</h2>
            <div className="w-16 h-1 bg-accent mx-auto mt-4 rounded-full"></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Rahul Sharma', reviewKey: 'testimonial1Text' },
              { name: 'Priya Patel', reviewKey: 'testimonial2Text' },
              { name: 'Amit Kumar', reviewKey: 'testimonial3Text' },
            ].map((tItem, i) => (
              <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, j) => <Star key={j} className="w-5 h-5 fill-accent text-accent" />)}
                </div>
                <p className="text-slate-600 mb-6 italic">{t(tItem.reviewKey)}</p>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-white font-heading text-xl">
                    {tItem.name[0]}
                  </div>
                  <h4 className="font-bold text-slate-800">{tItem.name}</h4>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="contact" className="py-20 bg-bg px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-4xl font-heading text-primary mb-6">{t('locationContact')}</h2>
            <p className="text-slate-600 mb-8 leading-relaxed">{t('contactDesc')}</p>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-800">{t('address')}</h4>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-800">{t('openingHours')}</h4>
                  <p className="text-slate-600">{t('monSatHours')}<br/>{t('sunHours')}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-800">{t('contactNumber')}</h4>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm opacity-80">
              ...
              <li><a href="#packages" onClick={(e) => { e.preventDefault(); scrollToSection('packages'); }} className="hover:text-white transition-colors">{t('packages')}</a></li>
              <li><a href="#services" onClick={(e) => { e.preventDefault(); scrollToSection('services'); }} className="hover:text-white transition-colors">{t('testFinder')}</a></li>
          ...
          <div>
            <h4 className="font-bold text-white mb-4">{t('contactInfo')}</h4>
            <ul className="space-y-2 text-sm opacity-80">
              <li>+91 6396786939</li>
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-50">
        ...
        >
          ...
          <span className="font-bold whitespace-nowrap hidden group-hover:block transition-all text-sm">{t('bookTestFloating')}</span>
        </a>
      </div>
