...
const ReportLookup = () => {
  ...
  const computeTrends = (reportsList) => {
    ...
    sortedReports.forEach(r => {
      ...
      r.results.forEach(res => {
        ...
        if (res.parameterName && !isNaN(val)) {
          ...
          parameterMap[name].history.push({
            date: dateStr,
            value: val,
    ...
    return Object.values(parameterMap).filter(p => p.history.length >= 2);
  };

  useEffect(() => {
    if (reports && reports.length > 0) {
      const computed = computeTrends(reports);
      setTrends(computed);
      if (computed.length > 0) {
        setSelectedTrend(computed[0].parameterName);
      } else {
        setSelectedTrend('');
      }
    } else {
  ...
  useEffect(() => {
    ...
    if (reportParam) {
      ...
      setReportNumberInput(reportParam);
      if (nameParam) {
        setPatientName(nameParam);
        handleDirectLookup(null, reportParam, nameParam);
      }
    } else if (mobileParam) {
      setSearchType('mobile');
      setQuery(mobileParam);
      fetchReports(mobileParam);
    }
  ...
  const handleDirectLookup = async (e, customRepNum = null, customPatName = null) => {
    if (e) e.preventDefault();
    const repNum = (customRepNum !== null ? customRepNum : reportNumberInput).trim();
    const patName = (customPatName !== null ? customPatName : patientName).trim();
    if (!repNum || !patName) return;
    ...
    try {
      const res = await fetch(`${API}/public/report-direct-lookup?reportNumber=${encodeURIComponent(repNum)}&patientName=${encodeURIComponent(patName)}`);
      const data = await res.json();
      if (!res.ok) {
        setError(data.message || 'Report not found or patient name does not match.');
      } else {
        setReports(data);
  ...
  const fetchReports = async (mobileNum) => {
    if (!mobileNum || !mobileNum.trim()) return;
    setLoading(true);
    setError('');
    setReports([]);
    try {
      const res = await fetch(`${API}/public/report-lookup?mobile=${encodeURIComponent(mobileNum.trim())}`);
      const data = await res.json();
      if (!res.ok) {
        setError(data.message || 'No reports found.');
      } else {
        setReports(data);
        if (data.length === 1) setExpandedId(data[0].id);
      }
  ...
  const handleExplainReport = async (report, groups, lang = aiLanguage) => {
  ...
  const renderStatusTracker = (status) => {
    const steps = [
      ...
      { label: 'Sample Collected', completed: true },
      { label: 'In Process', completed: true },
      { label: 'Verified', completed: status === 'COMPLETED' },
      { label: 'Ready to Download', completed: status === 'COMPLETED' }
    ];

    return (
      <div className="w-full py-4 px-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6">
        <div className="flex flex-col sm:flex-row justify-between gap-3 text-left sm:text-center">
          {steps.map((st, idx) => (
            <div key={idx} className="flex items-center sm:flex-col gap-2 sm:flex-1 relative">
              ...
              {idx < steps.length - 1 && (
                <div className={`hidden sm:block absolute top-3 left-[60%] w-[80%] h-[2px] -z-0 ${
                  steps[idx+1].completed ? 'bg-emerald-500' : 'bg-slate-200'
                }`} />
              )}
            </div>
          ))}
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 relative z-10">
        ...
        >
          ...
          <div className="text-center mb-8 relative">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight font-heading">
              Check Your Test Reports
            </h2>
            <p className="text-slate-500 mt-2 text-sm max-w-lg mx-auto">
              {searchType === 'mobile' ? (
                <>Enter your registered <span className="font-bold text-primary">mobile number</span> to instantly access your NABL verified laboratory reports.</>
              ) : (
                <>Enter your exact <span className="font-bold text-primary">Report Number</span> and <span className="font-bold text-primary">Patient Name</span> to instantly retrieve your verified laboratory report.</>
              )}
            </p>
          </div>
          ...
          <div className="flex justify-center mb-8 relative">
            <div className="bg-slate-100/80 backdrop-blur-sm p-1 rounded-2xl border border-slate-200 shadow-inner inline-flex w-full max-w-md">
              <button
                type="button"
                onClick={() => {
                  setSearchType('mobile');
          ...
          {searchType === 'mobile' ? (
            <form onSubmit={handleMobileLookup} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
              ...
              >
                ...
                {loading ? 'Retrieving Reports...' : 'View Reports'}
              </button>
            </form>
          ) : (
            <form onSubmit={handleDirectLookup} className="flex flex-col gap-4 max-w-xl mx-auto animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="Report Number (e.g. SPL-0001)..."
                    className="relative w-full pl-5 pr-5 py-4 border-2 border-slate-200 rounded-2xl text-sm font-semibold focus:outline-none focus:border-primary focus:ring-0 transition-all shadow-inner bg-white/80 backdrop-blur-sm"
                    required
                  />
                </div>
              ...
              >
                {loading ? <Loader type="button" className="text-white" /> : <Search className="w-4 h-4 animate-pulse-soft" />}
                {loading ? 'Retrieving Report...' : 'View Report'}
              </button>
            </form>
          )}
        ...
        {reports.length > 0 && (
          <div className="space-y-6 animate-slide-up mb-10">
            ...
            {trends.length > 0 && (
              <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_10px_35px_rgba(15,110,86,0.03)] p-6 mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-2">
                    <div className="p-2 bg-emerald-50 rounded-lg text-primary"><Activity className="w-5 h-5" /></div>
                    <div>
                      <h3 className="text-base font-black text-slate-800 uppercase tracking-wide">🧬 Health History Tracker</h3>
                      <p className="text-xs text-slate-400">Plot historical numerical parameters from your reports</p>
                    </div>
                  </div>
                  <select 
                    className="bg-slate-50 border border-slate-200 text-sm font-semibold rounded-lg px-3 py-2 text-slate-700 outline-none focus:ring-2 focus:ring-primary transition-all cursor-pointer"
                    value={selectedTrend}
                ...
                <div className="h-64">
                  {trendLineData && <Line data={trendLineData} options={chartOptions} />}
                </div>
                ...
                {activeTrend && (
                  <div className="mt-4 p-3 bg-slate-50 rounded-xl flex items-center justify-between text-xs font-semibold text-slate-600 border border-slate-100">
                    <span>Normal Clinical Reference Range:</span>
                    <span className="text-slate-800 font-bold bg-white px-2.5 py-1 rounded-lg border border-slate-100 font-mono">
            ...
            {reports.map((report, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-100 p-6 animate-fade-in-up">
                        ...
                        <>
                          ...
                          <div className="px-6 py-5 bg-gradient-to-r from-slate-50 to-white border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="flex items-center gap-3">
                             ...
                             <button onClick={() => {
                              const printUrl = `${window.location.origin}${import.meta.env.BASE_URL}#/public-print/${report.reportNumber}?token=${token}&patientName=${encodeURIComponent(report.patient?.fullName || '')}`;
                          ...
                          <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-center">
                            <button
                              onClick={() => handleExplainReport(report, groups, aiLanguage)}
                          ...
                          {/* AI Explanation Box */}
                          {(explanation || (isExplaining && explainingReportId === report.id)) && (
                            <div className="mx-6 mb-6 mt-2 relative animate-fade-in-up">
                              ...
                              <div className="relative bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-indigo-100 shadow-sm animate-fade-in-up">
                                <button 
                                  onClick={() => { setExplanation(''); setExplainingReportId(null); }}
                                  className="absolute top-4 right-4 p-1 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors"
                                >
                                  <X className="w-4 h-4" />
                                </button>
                                
                                <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pr-6">
                                  <div className="flex items-center gap-2">
                                    <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2 rounded-xl text-white shadow-sm animate-pulse-soft">
                                  ...
                                  <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner">
                                    <button 
                                      onClick={() => {
                                        setAiLanguage('en');
                                        handleExplainReport(report, groups, 'en');
                                      }}
                                    ...
                                    <button 
                                      onClick={() => {
                                        setAiLanguage('hi');
                                        handleExplainReport(report, groups, 'hi');
                                      }}
                                      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 ${
                                        aiLanguage === 'hi' 
                                          ? 'bg-white text-indigo-700 shadow-sm font-extrabold scale-[1.02]' 
