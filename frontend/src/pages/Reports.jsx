        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 p-6">
              {/* Status and Referring Doctor */}
              <div className="flex flex-col sm:flex-row gap-4 mb-6">
                <div className="flex-1 p-4 bg-gray-50 rounded-lg flex items-center gap-4">
                ...
                {user?.userType === 'STAFF' && (
                  <div className="flex-1 p-4 bg-gray-50 rounded-lg flex items-center gap-4">
                    <span className="text-sm font-bold text-gray-600 whitespace-nowrap">Referring Doctor:</span>
                    <div className="flex-1">
                      <Select
                        options={doctors.map(d => ({ value: d.id, label: `${d.doctorId} - ${d.name}` }))}
                        value={editDoctor}
                              ...
                              {params.map((res, idx) => (
                                <tr key={res.id || res.originalIndex} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                                  ...
                                  <td className="px-4 py-2">
                                    {res.parameterName?.toUpperCase().includes('TYPHIDOT') ? (
                                      <select
