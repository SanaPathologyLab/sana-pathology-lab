...
const Dashboard = () => {
  ...
  const dailyLabels = stats.dailyRevenueTrend?.map(t => t.date) || [];
  const dailyData = stats.dailyRevenueTrend?.map(t => t.amount) || [];
  ...
  const patientData = {
    ...
    datasets: [{
      label: 'Patients',
      data: Array(dailyLabels.length).fill(stats.totalPatients || 0),
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <div className="space-y-3">
            {[
              { label: 'Total Revenue', value: `₹${(stats.totalRevenue || 0).toLocaleString()}`, color: 'text-green-600 font-extrabold' },
              { label: 'This Month Revenue', value: `₹${(stats.monthRevenue || 0).toLocaleString()}`, color: 'text-blue-600 font-bold' },
              { 
                label: 'Last Month Revenue', 
                value: `₹${(stats.momComparison?.lastMonth || 0).toLocaleString()}`, 
                color: 'text-slate-600' 
              },
              { 
                label: 'MoM Growth', 
                value: (() => {
                  const cur = stats.momComparison?.currentMonth || 0;
                  const prev = stats.momComparison?.lastMonth || 0;
                  const diff = cur - prev;
                  const pct = prev > 0 ? ((diff / prev) * 100).toFixed(1) : (cur > 0 ? '100.0' : '0.0');
              ...
              <div key={i} className="flex justify-between items-center py-1.5 border-b border-gray-100 last:border-0">
                ...
                {item.isElement ? (
                  ...
                  <span className={`text-sm ${item.color || 'text-slate-800'}`}>{item.value}</span>
                )}
              </div>
            ))}
          </div>
