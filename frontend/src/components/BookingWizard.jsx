...
const BookingWizard = ({ existingCart, onCartUpdate, scrollToSection, isModal = false, onCloseModal, initialStep = 0 }) => {
  ...
  const renderActiveStep = () => {
    if (bookingSuccess) return successContent;
    switch (step) {
      case 0:
        return (
          ...
          <div className="animate-fade-in-up space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <input type="date" value={dateTime.date} min={todayStr} onChange={e => setDateTime({ ...dateTime, date: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all" />
  ...
  if (isModal) {
    return (
      <div className="w-full flex flex-col md:flex-row bg-white relative max-h-[85vh]">
        {/* Left Column: Form & Indicator */}
        <div className="flex-1 p-5 sm:p-8 overflow-y-auto relative max-h-[85vh]">
          {!bookingSuccess && stepIndicator}
          {renderActiveStep()}
        </div>

        {/* Right Column: Sidebar */}
        <div className="w-full md:w-80 bg-slate-50 p-6 md:p-8 border-l border-slate-200 overflow-y-auto hidden md:block shrink-0 max-h-[85vh]">
          {summarySidebar}
